...
class AIService:
    ...
    def _initialize_knowledge_base(self):
        try:
            ...
            if os.path.exists(docs_path):
                ...
                llm = OpenAI(model="gpt-4", temperature=0.1, api_key=settings.openai_api_key)
                embed_model = OpenAIEmbedding(api_key=settings.openai_api_key)
                service_context = ServiceContext.from_defaults(llm=llm, embed_model=embed_model)
                self.knowledge_base = VectorStoreIndex.from_documents(documents, service_context=service_context)
                logger.info("Knowledge base initialized successfully")
            else:
                logger.warning("Fitness docs directory not found, knowledge base disabled")
        except Exception as e:
            logger.error(f"Failed to initialize knowledge base: {e}")

    async def generate_workout_plan(self, user_profile: UserProfileResponse, week: str) -> WorkoutPlan:
        """Generate personalized workout plan using AI and external knowledge"""
        try:
            # Build context from user profile
            context = self._build_user_context(user_profile)
            
            # Get comprehensive fitness knowledge
            knowledge_query = KnowledgeQuery(
                query=f"workout plan for {user_profile.goals[0] if user_profile.goals else 'general fitness'}",
                sources=["internal", "exercise_db", "wger"],
            ...
            # Create enhanced prompt with external data
            prompt = self._create_workout_prompt(user_profile, week, context, knowledge_response)
    ...
    async def generate_meal_plan(self, user_profile: UserProfileResponse, week: str) -> MealPlan:
        """Generate personalized meal plan using AI and external nutrition data"""
        try:
            # Build context from user profile
            context = self._build_user_context(user_profile)
            
            # Get comprehensive nutrition knowledge
            knowledge_query = KnowledgeQuery(
            ...
            # Create enhanced prompt with nutrition data
            prompt = self._create_meal_prompt(user_profile, week, context, knowledge_response)
    ...
    async def adapt_plans(self, user_profile: UserProfileResponse, feedback_analysis: FeedbackAnalysis) -> Dict[str, Any]:
        """Adapt plans based on feedback analysis using comprehensive knowledge"""
        try:
            # Get relevant knowledge for adaptation
            knowledge_query = KnowledgeQuery(
            ...
            prompt = self._create_adaptation_prompt(user_profile, feedback_analysis, knowledge_response)
    ...
    async def analyze_feedback(self, feedback_summary: FeedbackSummary) -> FeedbackAnalysis:
        """Analyze feedback using AI and external knowledge"""
        try:
            # Get knowledge about feedback interpretation
            knowledge_query = KnowledgeQuery(
                query="workout feedback analysis and interpretation",
                sources=["internal"],
                include_nutrition=False,
                include_exercises=False
            )
            ...
            prompt = self._create_feedback_analysis_prompt(feedback_summary, knowledge_response)
    ...
    async def search_knowledge(self, query: str, sources: List[str] = None) -> KnowledgeResponse:
        ...
        if sources is None:
            sources = ["internal", "usda", "exercise_db", "wger"]
        ...
        return await self.external_api_service.get_comprehensive_knowledge(knowledge_query)
    ...
    def _create_workout_prompt(self, user_profile: UserProfileResponse, week: str, 
                             context: str, knowledge_response: KnowledgeResponse) -> str:
        """Create enhanced workout generation prompt"""
        prompt = f"""
        Generate a personalized workout plan for week {week}.
        
        User Context: {context}
        
        Available Knowledge Sources: {', '.join(knowledge_response.sources_used)}
        
        If exercise data is available, use it to suggest specific exercises. Otherwise, use general exercise guidelines.
        ...
        5. Follow proper exercise progression
    ...
    def _create_meal_prompt(self, user_profile: UserProfileResponse, week: str, 
                          context: str, knowledge_response: KnowledgeResponse) -> str:
        """Create enhanced meal generation prompt"""
        ...
        If nutrition data is available, use it to suggest specific foods. Otherwise, use general nutrition guidelines.
        ...
        5. Provide meal timing recommendations
    ...
    def _create_adaptation_prompt(self, user_profile: UserProfileResponse, 
                                ...
                                knowledge_response: KnowledgeResponse) -> str:
        """Create plan adaptation prompt"""
        prompt = f"""
    ...
    def _create_feedback_analysis_prompt(self, feedback_summary: FeedbackSummary, 
                                       knowledge_response: KnowledgeResponse) -> str:
